...
const errResJSON = ({ statusCode, title, type, message, errors = null }: IErrorResJSON) => ({
...
const errorMiddleware = (err: any, req: Request, res: Response, next: NextFunction) => {
  ...
  if (err.statusCode === 422) {
    // UnprocessableEntity
    // return res.status(422).json(errResJSON(422, err?.message || 'Unable to process your request.'));
