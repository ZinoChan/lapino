  ...
  return (
    <div className="grid grid-cols-4 gap-2 items-center">
      {features.map((item, index) => (
        <button
          key={`${item.name}-${index}`}
          className="flex items-center rounded-lg hover:bg-gray-50 shadow-md justify-center py-4 bg-white space-x-4"
        >
          <span>{item.icon}</span>
          <span className="font-main text-xl ">{item.name}</span>
        </button>
      ))}
    </div>
  );
